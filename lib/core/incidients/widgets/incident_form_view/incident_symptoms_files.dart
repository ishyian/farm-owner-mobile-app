...
class IncidentSymptomsFiles extends StatefulWidget {
  const IncidentSymptomsFiles({super.key});

  @override
  State<IncidentSymptomsFiles> createState() => _IncidentSymptomsFilesState();
}

class _IncidentSymptomsFilesState extends State<IncidentSymptomsFiles> {
  @override
  Widget build(BuildContext context) {
    return Consumer<IncidentFormProvider>(
      builder: (context, provider, child) {
        return Column(
          children: [
            for (int i = 0; i < provider.selectedSymptoms.length; i++)
              buildSingleSymptom(provider, i),
          ],
        );
      },
    );
  ...
  Widget buildSingleSymptom(IncidentFormProvider provider, int i) {
    return Container(
      ...
      child: Column(
        children: [
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: const BoxDecoration(
                      shape: BoxShape.circle, color: kDarkPrimary),
                  child: Center(
                    child: Image.asset(
          ...
          if (provider.symptomsFiles[i] != null &&
              provider.symptomsFiles[i]!.isNotEmpty) ...[
            16.vertical(),
            SizedBox(
              height: 80,
              child: ListView(
